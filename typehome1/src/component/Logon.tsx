...
function Logon() {
    ...
    let logon=()=>{
        ...
        if( parseInt(userid)>=1 && parseInt(userid)<=10)
        {
            ...
            .then((res)=>{
                ...
                context.dispatch( {type:"LOGON", value:{userid:userid, 
                    username:username, isLogon:true, callBack:callBack}});  
                //첫번째 매개변수는 라우터에서 정의한 url이다. --비동기라서 타이밍이 안맞음 
                //상태변경이 나중에 벌어진다. 
                //첫번째 인자 -> 함수를 전달, 두번째인자로 시간 :1000이 1초임 
    
                setTimeout( ()=>{ navigate("/home", {})}, 1000);  //페이지 이동 
                setMsg("");      
            }) 
            .catch(error=>{
                console.log(error);
    ...
    let logout=()=>{
        context.dispatch( {type:"LOGOUT", value:{userid:"", 
             username:"", isLogon:false, callBack:callBack}});
    }
    ...
    return (<div>
        ...
        <h3 style={{"color":"red"}}>{msg}</h3>
         ...
         <button type="button" onClick={logon}>로그인</button>
